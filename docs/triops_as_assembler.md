...
Furthermore, we could imagine an opaque fat pointer type (where 'opaque' means its elements are not directly named or indexable as they would be in a struct or array),
...
```
This can be read '8 by 2 (so 16 total) bytes'. Alignment is limited to powers of two, upto 256 bytes.
...
- `#stringform`
- `#byteform`
This form annotation is placed after the `bytes` keyword, so for our `int`:
```
type int is 4 bytes #intform;
```
...
As we saw in the introduction, variables 'like C', but only superficially.
However, in C, the type is actually declared _around_ the variable name, so that declaration is equal to usage, whereas in Triops, the type is really placed _before_ the name.
Using our trusty `int`, variables can be declared without explicit initialization (which means they are set to 0), or with initialization:
```
int a;
int b = 7;
```
...
type s32 is int;
```
`s32` is a distinct type, but it copied over the size, alignment and form annotation from `int`.
...
```
This means that a `vec3` variable can be assigned to and indexed as an array, but only when all elements are either floating point literals, or specifically `float`.
...
and its size is therefore element count of array times element count of type times alignment of type.
...
To use an enum, the structure is the enum name, a period, and the subvalue name:
```
int a = numbers.three;
```
